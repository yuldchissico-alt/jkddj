...
const HOTMART_ICON = (
  <svg width="27" height="37" viewBox="0 0 27 37" fill="none" xmlns="http://www.w3.org/2000/svg" className="size-5">
...
const PLATFORM_CONFIG = {
  ...
  hotmart: { label: "Hotmart", logo: HOTMART_ICON, type: "svg" as const },
...
export function PlatformLogo({ platform, size = "md", showLabel = true }: PlatformLogoProps) {
  ...
  if (config.type === "svg") {
    return (
      <div className="flex items-center gap-1.5">
        <div className={`${sizeMap[size]} flex items-center justify-center shrink-0`}>{config.logo}</div>
        {showLabel && (
