...
interface StripeTableProps {
  accounts: StripeAccountAPI[];
  isLoading: boolean;
  onDelete: (account: StripeAccountAPI) => void;
}
...
export function StripeTable({ accounts, isLoading, onDelete }: StripeTableProps) {
      ...
      <Card className="border-border/40 border-dashed">
        <CardContent className="flex flex-col items-center justify-center py-16 text-center gap-2">
          ...
          <p className="text-xs text-muted-foreground/70">
            Acesse Developers → API Keys no Stripe Dashboard para obter sua Secret Key.
          </p>
        </CardContent>
      </Card>
    );
